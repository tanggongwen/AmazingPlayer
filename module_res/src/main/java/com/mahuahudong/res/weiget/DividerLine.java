...
public class DividerLine extends RecyclerView.ItemDecoration {
    ...
    public DividerLine(Context context) {
        mContext = context;
        //获取样式中对应的属性值
        setDividerDrawable(context);

    }

    private void setDividerDrawable(Context context){
        if (lineType==0){
            dividerDrawable = context.getResources().getDrawable(R.drawable.shape_divider);
        }else if (lineType==1){
            dividerDrawable = context.getResources().getDrawable(R.drawable.shape_divider_gray);
        }
    }
    ...
    public DividerLine(Context context, int dividerSize, LineDrawMode mode,int lineType,int horizionMargin,int verticalMargin) {
        this(context, mode);
        this.dividerSize = dividerSize;
        this.lineType = lineType;
        this.horizontalMargin = horizionMargin;
    ...
    private void drawVertical(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            final RecyclerView.LayoutParams params = (RecyclerView.LayoutParams) child
                    .getLayoutParams();
            final int top = child.getTop() - params.topMargin-verticalMargin;
            final int bottom = child.getBottom() + params.bottomMargin+verticalMargin;
            final int left = child.getRight() + params.rightMargin+horizontalMargin;
    ...
    private void drawHorizontal(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            //child的左边缘(也是分隔线的左边)
            final int left = child.getLeft() - params.leftMargin-horizontalMargin;
            //child的底边缘(恰好是分隔线的顶边)
            final int top = child.getBottom() + params.topMargin+verticalMargin;
            //child的右边(也是分隔线的右边)
            final int right = child.getRight() - params.rightMargin-horizontalMargin;
