...
public class PhoneReceiver extends BroadcastReceiver {
    ...
    private void seekToMusic(int seekProgress) {
        if (AudioStateManager.getInstance().getCurAudioMessage() != null && AudioStateManager.getInstance().getCurAudioInfo() != null) {
            int progress = (int) (AudioStateManager.getInstance().getCurAudioMessage().getPlayProgress() + seekProgress);
            //判断歌词快进时，是否超过歌曲的总时间
            if (Integer.parseInt(AudioStateManager.getInstance().getCurAudioInfo().getAudioTime()) < progress) {
                progress = Integer.parseInt(AudioStateManager.getInstance().getCurAudioInfo().getAudioTime());
