...
public class DownloadTaskManager {
    ...
    public DownloadTaskManager(Context context, boolean isTheOneTaskThreadFristStart, IDownloadTaskEvent interfaceEvent) {
        mainThread.start();
    ...
    public synchronized void addMultiThreadMultiTask(final DownloadTask task) throws Exception {
        new Thread() {
            @Override
            public void run() {
                TaskThreadUtil taskThreadUtil = new TaskThreadUtil(context, task, taskEvent,
...
//        if (tasks.size() > 0) {
        DownloadTask temp = tasks.get(0);
        Log.e("download","isWaiting = " + isWaiting + " 任务当前状态是否是初始化：" + (temp.getStatus() == DownloadTaskConstant.INT.getValue()));
        // 如果等于初始化
    ...
    protected synchronized void startTaskThread() {
        if (tasks.size() > 0) {
            ...
            if (task.getStatus() == DownloadTaskConstant.WAIT.getValue()) {
                ...
                taskThreadUtil.startTask(context);
                Log.e("download",task.getTaskName() + " 任务正在下载，任务id是：" + task.getTaskId());
            }
